...
exports.commands = {
	...
	sggame: {
      ...
      if (target[0] === 'trade') {
				...
				if (!player.party[target[1]]) return this.parse('/sggame pokemon');
				let tradePoke = player.party[opts[0]]
				if (!tradePoke) return this.parse('/sggame pokemon summary, ' + opts[0]);
                let to = Db.players.get(opts[1].replace(/[^A-Z0-9]/gi, "").toLowerCase())
                 let toConsole = Users(to.userid)
                 toConsole.update(null,toConsole.console.askfortrade(user.name,tradePoke))
				return this.parse('/sggame pokemon summary, ' + target[1]);
			}
