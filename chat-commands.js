...
const commands = {
  ...
  makegroupchat: function(target, room, user, connection, cmd) {
    ...
    let titleMsg = Chat.html`Welcome to ${parent ? room.title : user.name}'s${
      !/^[0-9]+$/.test(title) ? ` ${title}` : ""
    }${parent ? " subroom" : ""} groupchat!`;
  ...
  warn: function(target, room, user) {
    ...
    const global =
      room.id === "staff" ||
      room.id.startsWith("help-") ||
      (room.battle && !room.parent);
  ...
  globalban: function(target, room, user, connection, cmd) {
    ...
    for (const roomid of targetUser.inRooms) {
      ...
      if (
        targetRoom.isPersonal &&
        targetRoom.auth[userid] === Users.HOST_SYMBOL
      ) {
        targetRoom.destroy();
      }
    }
  ...
  modnote: function(target, room, user, connection) {
    ...
    if (room.id === "staff" || room.id === "upperstaff")
      this.globalModlog("NOTE", null, ` by ${user.userid}: ${target}`);
  ...
  killhelp: [
    `/kill - kills the server. Can't be done unless the server is in lockdown state. Requires: ~`
  ],
  ...
  editbattle: function(target, room, user) {
    ...
    switch (cmd) {
      ...
      case "fc":
        room.battle.stream.write(
          `>eval battle.addPseudoWeather('${toId(targets[0])}', 'debug')`
        );
        break;
      case "weather":
      case "w":
        room.battle.stream.write(
          `>eval battle.setWeather('${toId(targets[0])}', 'debug')`
        );
        break;
      case "terrain":
      case "t":
        room.battle.stream.write(
          `>eval battle.setTerrain('${toId(targets[0])}', 'debug')`
        );
        break;
      default:
  ...
  a: function(target, room, user) {
	...
	'!discord': true,	discord(target, room, user) {		if (!this.runBroadcast()) return;		this.sendReplyBox(`<a href="https://discord.gg/bE98zxY">The Official ${Config.serverid} Discord</a>`);	},
  /*********************************************************
   * Help commands
   *********************************************************/
