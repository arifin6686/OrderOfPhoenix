...
class ExpFunctions {
	...
	grantExp(room, amount,) {
		Users.users.forEach(user => {
			if (!user || !user.named || !user.connected || !user.lastPublicMessage) amount = 1; return;
			if (Date.now() - user.lastPublicMessage > 300000) amount = 1;return;
			this.addExp(user, null, amount);
		});
	}
